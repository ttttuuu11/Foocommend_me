...
<body>
 ...
 -->
	...
	<script>
		$(function() {
			$(".restaurantCard").on("click", function() {
				restaurantIdx = $(this).children(":eq(0)").val()
				location.href = "/board/detail?restaurantIdx="+restaurantIdx;
			});
			$(".restaurantCard").hover(function(){
				$(this).css("border","solid 3px #ffaaaa");
			},function(){
	...
	<div class="row">
		...
		<div class="card-columns col-sm-12 col-md-12 col-lg-9 col-xl-9">
			<c:forEach var="recommentItem" items="${resultRecommendList }"
				varStatus="status">
				<div class="card p-3 shadow restaurantCard">
					<input type="hidden"
						value="${restaurantIntroList[status.index].restaurant_idx}" /> <img
						class="card-img-top"
					...
					<div class="card-body">
						...
						<small>${restaurantIntroList[status.index].restaurant_street_add}</small>
						<hr>
